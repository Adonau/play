function pageslider(item, callback) {
	...
	var $this			= item,
		...
		$navLi			= document.getElementById('page-slider-list').children, // id of main nav > li
	...
	$pageActive.classList.add('intro__active');
	...
	function sliderActive(pageActive) {
		...
		if ($navLi !== null) {
			$navLi[pageActive].classList.add('item-active');
		}
	...
	function classClean() {
		...
		for (var i = pageCount; i >= 0; i--) {
			$page[i].classList.remove('intro__active');

			if ($navLi !== null) {
				$navLi[i].classList.remove('item-active');
			}
		};
	}
		...
		if (nowHandle - lastCallHandle > delay) {
			...
			if ( delta > 0 ) {
				...
				if (nav) {
					...
					$noActivePage.classList.add('intro__active');
	...
	//add active class for new slider
		...
		for (var i = pageCount; i >= 0; i--) {
			...
			// for ie
			pageClassList.remove('intro__active');
